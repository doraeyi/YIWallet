...
// sop app 沒有自己的登入頁，未登入時會帶 return_to 導來這裡；登入完（帳密或 Google）都要導回去
...
function GoogleLoginLink() {
  const searchParams = useSearchParams()
  const returnTo = searchParams.get('return_to')
  const href = returnTo ? `/api/auth/google/redirect?return_to=${encodeURIComponent(returnTo)}` : '/api/auth/google/redirect'
  return (
    <a
      href={href}
