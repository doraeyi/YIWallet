...
export async function GET(req: NextRequest) {
  ...
  const returnTo = jar.get('google_return_to')?.value // 從 sop app 被導來登入時帶的原始網址
  ...
  if (mode === 'link') {
    ...
    let res: Response
    try {
      res = await fetch(`${process.env.API_URL}/users/me/google`, {
        method: 'POST',
        headers: {
      ...
      console.error('[google callback] users/me/google fetch failed:', err)
      redirect('/settings?error=google_failed')
    }
  ...
  try {
    backendRes = await fetch(`${process.env.API_URL}/auth/google`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ access_token: accessToken }),
    })
  } catch (err) {
    console.error('[google callback] backend /auth/google fetch failed:', err)
